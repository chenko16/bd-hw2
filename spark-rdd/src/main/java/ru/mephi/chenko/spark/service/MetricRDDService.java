...
public class MetricRDDService {
     ...
     */
    public static JavaRDD<AggregatedMetricDto> aggregateMetric(JavaRDD<MetricDto> rdd, Integer minutes) {
        return rdd
                .map(metric -> {
                    metric.setTime(DateUtil.round(metric.getTime(), minutes));
                    return metric;
                })
                ...
                .map(pair -> new AggregatedMetricDto(pair._1._1, pair._1._2, minutes + "m", pair._2));
    }
}
